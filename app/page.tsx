...
export default function SalesDashboard() {
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 text-lg">Error loading data. Please make sure data.json exists in the public folder.</p>
        </div>
      </div>
    );
  }
  ...
  const filteredData = allSales.filter(sale => {
    ...
    const priceMatch = minPrice ? sale.price >= parseInt(minPrice) : true;
    const emailMatch = customerEmail ? sale.customerEmail.toLowerCase().includes(customerEmail.toLowerCase()) : true;
    const phoneMatch = phoneNumber ? sale.customerPhone.includes(phoneNumber) : true;

    return dateInRange && priceMatch && emailMatch && phoneMatch;
  });
            ...
            <>
              ...
              <div className="px-6 py-4 border-t border-slate-200 flex justify-between items-center flex-wrap gap-4">
                <div className="text-sm text-slate-600">
                  ...
                  <span className="font-semibold">{Math.min((currentPage + 1) * itemsPerPage, sortedData.length)}</span> of{' '}
                  <span className="font-semibold">{sortedData.length}</span> results (Page {currentPage + 1} of {totalPages})
                </div>
